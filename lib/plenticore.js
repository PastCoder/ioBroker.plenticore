...
function setup(callback) {
	...
	batteryControlMode = getBatteryControlMode();
...
function getBatteryControlMode() {
	let batteryControlMode = 1;

	adapter.getState('devices.local.battery.SmartBatteryControlMode', function(err, state) {
		if(!err && state) {
			batteryControlMode = state.val;
		}
	});
	return batteryControlMode;
}
			...
			adapter.getState('forecast.consumption.remaining', function(err, state) {
				...
				adapter.getState('devices.local.battery.SoC', function(err, state) {
					...
					adapter.getState(useMinSoCId, function(err, state) {
						...
						if ( (batteryControlMode == 1) && (batteryStrategy == '2') ) {
...
function setMinHomeConsumption(value) {
	// write battery.MinHomeConsumption
	adapter.getState('devices.local.battery.MinHomeConsumption', function(err, state) {
		if(!err) {
			let curValue = (state ? state.val : null);

			if(curValue === null || curValue !== value) {
				adapter.log.info('Set in converter MinHomeConsumption to ' + value);
				adapter.setState('devices.local.battery.MinHomeConsumption', value, false);
			}
		}
	});
}
...
function changeSetting(id, value) {
	let moduleid;
	let settingid;
	let normalMinConsumption = 50; //TODO: make this value configurable

	// check first the special cases which should be transferred to the plenticore device
	if (id == 'devices.local.battery.SmartBatteryControlMode') {
		...
		batteryControlMode = value;

		if (batteryControlMode == 1) {
			setMinHomeConsumption(normalMinConsumption);  // set back to normal value to use the battery power
		}
		else if (batteryControlMode == 2) {
			setSmartBatteryControl(false);
			setMinHomeConsumption(normalMinConsumption);  // set back to normal value to use the battery power
		} else if (batteryControlMode == 3) {
			setSmartBatteryControl(true);
			setMinHomeConsumption(normalMinConsumption);  // set back to normal value to use the battery power
		} else if (batteryControlMode == 4) {
			setSmartBatteryControl(false);
			setMinHomeConsumption(10000);  // high value to never use power from battery
		} else if (batteryControlMode == 5) {
			// TODO: Add AC loading (do not forget to update every 3 minutes)
			//setSmartBatteryControl(false);
			//setMinHomeConsumption(10000);  // high value to never use power from battery
		}
