			...
			for(let shindex in sunhour_power) {
				...
				if(fcPower) {   
					...
					if (factor > 0) {    // only if the period is relevant
						...
						// To avoid toggle between true and false a debounce is implemented:
						//   below 70% of maxFeedInPower --> set false
						//   above 90% of maxFeedInPower --> set true
						//   between 70% and 90% --> keep previous value
						// Consider that it must be consolidated across all relevant hours of the current day: If at least one hour exceeds the limitation, then feed_in_limitation_expected should be set to true

						if( fcPower > (maxFeedInPower * 0.90) ) {     
							adapter.log.debug('(battery management) fcPower ' + fcPower + ' for hour ' + shindex + ' (starts at ' + time + ') is above 90% maximum feed-in power  ' + (maxFeedInPower * 0.90) + ' --> set feed_in_limitation_expected to true.');
							reasonForOn = true; // there is at least one hour, which causes us to switch on
						} else if ( fcPower > (maxFeedInPower * 0.70) ) {     
							adapter.log.debug('(battery management) fcPower ' + fcPower + ' for hour ' + shindex + ' (starts at ' + time + ') is above 70% maximum feed-in power  ' + (maxFeedInPower * 0.90) + ' --> keep feed_in_limitation_expected in true, if it was before.');
							reasonToKeep = true; // there is at least one hour, which causes us to keep it on (if it was already on before)
						}
					}
				}
			}
			
			// In the loop above we checked, if there is at least one hour, which motivates us to set feed_in_limitation_expected=true or to keep it on true
			if (reasonForOn == true) {
				feed_in_limitation_expected = true;   // at least one hour was above the upper limit (to set on)
			} else if (reasonToKeep == true) {
				// just keep the value as it was     // at least one hour was above the lower limit (to keep it on)
			} else {
				feed_in_limitation_expected = false; // all hours were below lower limit
			}
		}
