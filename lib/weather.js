...
adapter.log.debug('nnn 1111111111');
	if(match && match.length > 0) {
adapter.log.debug('nnn 222222');
        try {
            let json_rain;
			let json_snow;
...
adapter.log.debug('nnn 555555');
			return true;
        } catch(e) {
adapter.log.debug('nnn EEEE 111');
			adapter.log.warn('Processing rain data failed with exception: ' + match[1]);
            return false;
        }
    } else {
adapter.log.debug('nnn EEEE 222');
